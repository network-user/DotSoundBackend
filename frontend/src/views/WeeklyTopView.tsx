...
export function WeeklyTopView() {
  ...
  const shareUrl = `${window.location.origin}${import.meta.env.BASE_URL}weekly-top`

  const load = useCallback(() => {
    setLoading(true)
    api
      .getWeeklyTopPlaylist(50)
      .then(setData)
      .catch(() =>
        setData({
          tracks: [],
  ...
  return (
    <section className="view active">
      <div className="view-header">
        ...
        <div style={{ flex: 1 }}>
          <h2>{t('weeklyTop.title')}</h2>
          <span className="hint">{t('weeklyTop.hint')}</span>
        </div>
        <button
          className="icon-btn"
          type="button"
      ...
      {!loading && heroUrl && (
        <AmbientStage coverUrl={heroUrl} className="rh-mix-hero">
          <div className="rh-mix-hero__inner">
            <div>
              <h1 className="rh-mix-hero__title">{t('weeklyTop.title')}</h1>
              <p className="rh-mix-hero__hint">{t('weeklyTop.hint')}</p>
      ...
      >
        <TrackList
          tracks={tracks}
          emptyMessage={t('weeklyTop.empty')}
        />
      </m.div>
