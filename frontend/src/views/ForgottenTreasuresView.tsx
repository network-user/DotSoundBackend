...
export function ForgottenTreasuresView() {
  const { t } = useTranslation()
  const navigate = useNavigate()
  const { playTrack, toggleShuffle } = usePlayerActions()
  const { shuffleOn } = usePlayerMeta()
  const [data, setData] = useState<ForgottenTreasuresPlaylistResponse | null>(
    null,
  )
  ...
  const shareUrl = `${window.location.origin}${import.meta.env.BASE_URL}forgotten-treasures`
  ...
  usePrefetchTracks(tracks ?? null, 'forgotten_treasures')
  ...
  const handlePlayAllHero = useCallback(async () => {
    ...
    try {
      ...
      showIsland({
        kind: 'error',
        title: getApiErrorMessage(e, t('redesign.artist.playError')),
        durationMs: 4000,
      })
    }
  ...
  return (
    <section className="view active">
      <div className="view-header">
        ...
        <div style={{ flex: 1 }}>
          <h2>{t('forgottenTreasures.title')}</h2>
          <span className="hint">{t('forgottenTreasures.hint')}</span>
        </div>
        <button
          className="icon-btn"
          type="button"
      ...
      {!loading && heroUrl && (
        <AmbientStage coverUrl={heroUrl} className="rh-mix-hero">
          <div className="rh-mix-hero__inner">
            <div>
              <h1 className="rh-mix-hero__title">
                {t('forgottenTreasures.title')}
              </h1>
              <p className="rh-mix-hero__hint">{t('forgottenTreasures.hint')}</p>
              <div className="rh-mix-actions">
                <MotionPress
                  variant="primary"
                  onClick={() => {
                    void handlePlayAllHero()
                  }}
                >
