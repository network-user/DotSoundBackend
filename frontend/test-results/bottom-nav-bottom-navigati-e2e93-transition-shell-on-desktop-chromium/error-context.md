...
- Name: bottom-nav.spec.ts >> bottom navigation dock >> keeps the swapped order and page transition shell on desktop
- Location: e2e\bottom-nav.spec.ts:152:3
...
Call log:
  - waiting for getByRole('button', { name: 'Медиатека' })
    - locator resolved to <button tabindex="0" type="button" aria-label="Медиатека" class="mp-press mp-press--ghost rb-nav__btn">…</button>
...
```ts
  59  |     }
  60  | 
  61  |     if (path === '/api/v1/recommendations/home-highlight') {
  62  |       await route.fulfill({ json: null })
  63  |       return
  64  |     }
  65  | 
  66  |     if (path.startsWith('/api/v1/recommendations/home/sections/')) {
  67  |       await route.fulfill({
  68  |         json: {
  69  |           section_type: 'continue',
  70  |           title: 'Continue',
  71  |           subtitle: null,
  72  |           tracks: [],
  73  |           has_more: false,
  ...
  79  | 
  80  |     if (
  81  |       path === '/api/v1/users/me/listen-history' ||
  82  |       path === '/api/v1/users/me/library' ||
  83  |       path === '/api/v1/users/me/collection' ||
