...
## README и документация (DotCore)

Человеческая документация - [README.md](README.md) в стандарте DotCore:
плоский техдок, обложка `docs/cover.svg` + `<img>`, header-бейджи в одном
`<p>`, LoC-бейдж в маркерах `<!-- loc:start -->` / `<!-- loc:end -->`. Этот
`agents.md` - канон operational-правил; README его не дублирует.

- «Обнови README» / «сгенерируй README» → скилл `generate-readme`
  (обновляет README + `docs/cover.svg` + `.cursor/rules/dotcore-project.mdc`).
- **README-sync (глобальные изменения):** новые/удалённые команды или
  скрипты, новый/удалённый модуль или сервис, смена архитектуры, изменение
  зависимостей/стека, новая/убранная фича, смена runtime → агент параллельно
  обновляет `README.md` тем же скиллом, включая пересчёт LoC.
- **Не глобальные** (README не трогать): опечатки, внутренний рефактор без
  изменения команд/API, правки комментариев, мелкие багфиксы.
- Не латай разметку README вручную и не удаляй LoC-маркеры - перегенерируй
  скиллом. Соблюдай black-box PrivateCore и в README, и в обложке: opaque-
  формулировки, без имён внутренней реализации.
