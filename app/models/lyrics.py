...
class TrackLyrics(Base, TimestampMixin):
    ...
    synced_lines: Mapped[list | None] = mapped_column(JSON, nullable=True)
    source: Mapped[str] = mapped_column(
        String(20), nullable=False, server_default="manual"
    )
    source_name: Mapped[str | None] = mapped_column(String(50), nullable=True)
    sync_source_name: Mapped[str | None] = mapped_column(
        String(50), nullable=True
    )
    sync_quality: Mapped[str | None] = mapped_column(String(16), nullable=True)
